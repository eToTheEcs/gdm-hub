...
<html>
    ...
    <body lang="it">
		...
		<div class="container contentContainerBg">
			...
			<h1 class="heading">Il tuo profilo</h1>

			<div class="container-fluid my-5 py-5 articleContent" id="userDataContainer">
				<?php
					...
					while($row2 = mysqli_fetch_assoc($qres)) {
						array_push($mieiComuni, $row2["Comune"]);
					}

					$query = "SELECT IdComune, Nome
							  FROM comuni";
					$qres = mysqli_query($dbconn, $query);

					$tuttiComuni = array();
					$tuttiComuni["ids"] = array();
					$tuttiComuni["nomi"] = array();

					while($row3 = mysqli_fetch_assoc($qres)) {
						array_push($tuttiComuni["ids"], $row3["IdComune"]);
						array_push($tuttiComuni["nomi"], $row3["Nome"]);
					}

					$listaComuni = "";

					for($i = 0; $i < count($tuttiComuni["ids"]); $i++) {

						$idComune = $tuttiComuni["ids"][$i];
						$nomeComune = $tuttiComuni["nomi"][$i];

						$listaComuni .= "<div class='custom-control custom-checkbox text-left'>
											<input type='checkbox' ".((in_array($idComune, $mieiComuni)==TRUE) ? " checked ": "")."class='custom-control-input' id='comune_".$idComune."' value='".$idComune."' name='comuni[]'>
											<label class='custom-control-label' for='comune_".$idComune."'>".$nomeComune."</label>
										 </div>";
					}

					echo "<form id='changePw' class='form-group' action='updateComuni.php' method='post'>
								<div class='form-group row'>
									<label for='username' class='col-sm-2 col-form-label'>Username</label>
									<div class='col-sm-10'>
									  <input type='text'readonly class='form-control-plaintext' id='username' value='".$row["NomeUtente"]."'>
									</div>
								</div>
								<div class='form-group row'>
									<label class='col-sm-2 col-form-label' for='pw'>Password</label>
									<div class='col-sm-10'>
										<input class='form-control' id='pw' type='password' readonly value='".$row["Password"]."'/>
									</div>
								</div>
								<div class='form-group row'>
									<label class='col-sm-2 col-form-label' for='pw'>I tuoi comuni</label>
									<div class='col-sm-10'>
										".
											$listaComuni
										."
									</div>
									<input type='submit' class='form-control' name='sub' value='aggiorna comuni'/>
								</div>
							</form>";
				?>
			</div>
