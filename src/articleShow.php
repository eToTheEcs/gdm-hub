...
<html>
    ...
    <body lang="it" class="articleShowBg">
		...
		<div class="container contentContainerBg">
			...
			<?php
				...
				// receive article data and display it
				if($_SERVER["REQUEST_METHOD"] == "GET" && !empty($_GET["articleid"])) {
					...
					$query = "SELECT AR.*, AU.NomeAutore, CO.Nome
							  ...
							  WHERE AR.Autore = AU.IdAutore
							   		AND AR.Comune = CO.IdComune
							  		AND AR.IdArticolo = $articleId";
					...
					if(mysqli_affected_rows($dbconn) == 1) {
						...
						echo "<article class='mx-2'>";

						echo "<h1 class='heading'>".$row["Titolo"]."</h1>";
						echo "<small class='spicyText secondaryInfo'> Di ".$row["NomeAutore"]."</small>";
						echo "<h2 class='articleContent my-5'>".$row["Testo"]."</h2>";
			...
			<!-- comment section -->
			<div class="mx-2">
				<p class="h3 m-2 heading">Commenti</p>
				<!-- decido di inviare i dati del form con JavaScript+JQuery -->
				<form form="commentForm" id="commentForm" class="row comment-box-main p-3 rounded-bottom">
					<div class="col-md-9 col-sm-9 col-9 pr-0 comment-box">
						<textarea type="text" name="commentText" id="commentText" class="form-control" placeholder="dì quello che pensi" required></textarea>
					</div>
					<div class="col-md-3 col-sm-2 col-2 pl-0 text-center send-btn">
						<input type="submit" class="btn btn-info" value="Commenta"/>
					</div>
					<input type="hidden" name="userId" value="<?php echo $_SESSION["IdUtente"];?>" />
					<input type="hidden" name="articleId" value="<?php echo $articleId; ?>" />
				</form>
			</div>
			<div class="commentsContainer my-5" id="commentsContainer">
				<?php
					...
					if(mysqli_affected_rows($dbconn) != 0) {
						while ($row = mysqli_fetch_assoc($qres)) {
							echo "<div class='row comments mx-2'>
									<div class='col-md-9 col-sm-9 col-9 comment rounded mb-2'>
										<h4 class='m-0 heading'><a href='#'>".$row["NomeUtente"]."</a></h4>
						...
						echo "<h1 class='heading'>Nessun commento per questo articolo.</h1>";
					}
				?>
			</div>
			<!-- /comment section -->
