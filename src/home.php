...
<html>
    ...
    <body lang="it">
		...
		<div class="container wrapper" style="height:70%;">
			<!-- content header -->
			<div class="px-3 py-3 pt-md-5 pb-md-4 mx-auto text-center">
				<p class="h1 heading">
					Notizie scottanti ti aspettano 🔥
				</p>
				<p class="h5">
					queste sono le 10 notizie più recenti 😉.
				</p>
			</div>
			<!-- /content header -->

			<!-- slideshow -->
			<div class="px-3 py-3 pt-md-5 pb-md-4 mx-5 text-center" style="/*border: 1px solid black;*/">
				<p class="h1 heading" id="galleryTitle">
				</p>
				<p class="h4 text-left">
					<span class='spicyText secondaryInfo' id="galleryAuthor">
					</span>
				</p>
				<p class="h5 text-left" id="galleryDate">
				</p>
				<p class="h5 hint" id="galleryHint">
					ancora un attimo...
				</p>
			</div>
			<!-- /slideshow -->
	        ...
	        <?php
				...
				// track the user with cookies
				/*if(!isset($_COOKIE["userAuth"])) {

					$_SESSION["managerObj"]->addDate(Date("d-m-Y H:i:s"));
					$esit = setcookie("userAuth", $_SESSION["managerObj"]->toString(), time() + (MINUTE*120), "/");
				}
				else {

					$_SESSION["managerObj"]->addDate(Date("d-m-Y H:i:s"));

					$_COOKIE["userAuth"] = $_SESSION["managerObj"]->toString();
					$visite = json_decode($_COOKIE["userAuth"]);
				}*/
			?>
		</div>
		<!-- footer -->
	...
	<script type="text/javascript">
		...
		// send data to the crawlerHandler
		// get back the number of newly inserted articles
		// after that, fetch the hottest news :)
		$.ajax({
			type: "POST",
			url: "crawlerHandler.php",
			data: "comuni=" + 	<?php
									$comuni = array();
									$query = "SELECT Comune
											  FROM preferisce
									...
									while($row = mysqli_fetch_assoc($qres))
										array_push($comuni, $row["Comune"]);
									echo "\"". addslashes(serialize($comuni))."\"";
								?>,
			dataType: "html",
			success: function(data) {
				...
				if(!('Notification' in window)) {
					console.log("Web notifications API not suppored");
				}
				else {
					console.log("Web notifications API suppored!");

					var notifTitle = "GDM Hub";

					var msg = (data != 0) ? "Ci sono " + data + " nuovi articoli da leggere 😁" : "niente di nuovo 😞";
				...
				// AJAX call to fetch the most recent articles
				$.ajax({
					...
					success: function(data) {
						...
						for(let i = 0; i < articles.length; ++i) {
							slideshowData.push({
									...
									"municipality": articles[i].municipality
							});
						}
					...
					error: function(xhr, status, error) {
						...
						console.log("ERROR: unable to get latest news, error message: " + xhr.responseText);
					}
				});
			},
			error: function(xhr, status, error) {
				console.log("ERROR: unable to crawl the new articles, error message: " + xhr.responseText);
			}
		});

		// === manage the news slideshow ===

		var INTERVAL = 7000;
		var isGoing = true;	// slideshow status
		...
		function timeGallery() {
			...
			// delete text from the hint and make it invisible
			var hintNode = document.getElementById("galleryHint");
			hintNode.innerHTML = "";
			hintNode.style.display = "hidden";
			currentArticleIndex = (currentArticleIndex + 1) % slideshowData.length;
			swipe();
			timer = setTimeout(function() {timeGallery();}, INTERVAL);
		}

		// advance and load new data
		function swipe() {

			var titleNode = document.getElementById("galleryTitle");
			var dateNode = document.getElementById("galleryDate");
			var authorNode = document.getElementById("galleryAuthor");

			while(titleNode.hasChildNodes())
				titleNode.removeChild(titleNode.lastChild);
			var titleLink = document.createElement("a");
			titleLink.href = "articleShow.php?articleid="+slideshowData[currentArticleIndex].id;
			titleLink.innerHTML = utf8Decode(slideshowData[currentArticleIndex].title);
			titleLink.classList.add("titleLink");

			titleNode.appendChild(titleLink);

			dateNode.innerHTML = slideshowData[currentArticleIndex].date;

			if(slideshowData[currentArticleIndex].author == null || slideshowData[currentArticleIndex].author == "")
				authorNode.innerHTML = "<small class='spicyText secondaryInfo'>AA.VV.</small>";
			else
				authorNode.innerHTML = "di " + slideshowData[currentArticleIndex].author;
		}
