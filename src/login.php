...
<html lang="it">
	<head>
		<meta charset="utf-8">
		<title>Login to GDM hub</title>
		<link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/css/bootstrap.min.css" integrity="sha384-MCw98/SFnGE8fJT3GXwEOngsV7Zt27NXFoaoApmYm81iuXoPkFOJwJ8ERdknLPMO" crossorigin="anonymous">
		<link rel="stylesheet" href="..\res\styles\login_style.css"/>
	...
	<?php
		...
		// if user had already logged in, redirect him to the home page
		...
		if($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST["username"]) && !empty($_POST["password"])) {
			...
			// sanitize input
			$username = filter_var(filter_var(trim($_POST["username"]), FILTER_SANITIZE_STRING), FILTER_SANITIZE_SPECIAL_CHARS);
			$password = filter_var(trim($_POST["password"]), FILTER_SANITIZE_STRING);

			//echo "$username - $password";

			// prepare the query
			/*$preparedQuery = mysqli_prepare($dbconn, "SELECT *
											FROM utenti
			...
			echo mysqli_stmt_error($preparedQuery)."<br>";

			echo "<h1>".mysqli_stmt_affected_rows($preparedQuery)."</h1>";*/
			...
			$idUtente = mysqli_fetch_assoc(mysqli_query($dbconn, $query))["IdUtente"];

			//echo "<h1>".mysqli_affected_rows($dbconn)."</h1>";

			// NOTE: se metto == 1 non funziona....mistero
			if(mysqli_affected_rows($dbconn) == 0) {
				echo "<div class='alert alert-danger' role='alert'> Wrong email or password </div>";
			}
			else {
				header("Location: home.php");
				// save user credential in a session
	...
	<body>
		...
		<form class="form-signin" method="post" action="login.php">
			...
			<!-- LOGO -->
			<svg viewBox="0 0 215 26" class="mb-4">
			...
			<input type="text" name="username" class="form-control fieldData" placeholder="Username" required autofocus>

			<label for="inputPassword" class="sr-only">Password</label>
			<input type="password" name="password" id="inputPassword" class="form-control fieldData" placeholder="Password" required>
			...
			<p class="text-center">Non hai ancora un account? <a href="register.php">Registrati</a></p>
    	</form>
	</body>
</html>
